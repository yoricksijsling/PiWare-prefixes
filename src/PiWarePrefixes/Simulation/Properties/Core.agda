...
-- f ⟫ id ≡ f
seq-right-identity : ∀ {i o} (f : ℂ i o) → f ⟫ id⤨ ≈⟦⟧ f
seq-right-identity f = easy-≈⟦⟧ $ VE.from-≡ ∘ λ w → pid-id (⟦ f ⟧ w)

-- id ⟫ f ≡ f
seq-left-identity : ∀ {i o} (f : ℂ i o) → id⤨ ⟫ f ≈⟦⟧ f
...
-- id{0} || f ≡ f
par-left-identity : ∀ {i o} (f : ℂ i o) → id⤨ {0} ∥ f ≈⟦⟧ f
par-left-identity f = easy-≈⟦⟧ (λ w → VE.refl _)

-- f || id{0} ≡ f
par-right-identity : {i o : ℕ} (f : ℂ i o) → f ∥ id⤨ {0} ≈⟦⟧ f
par-right-identity f = Mk≈⟦⟧ (+-right-identity _) par-right-identity-≈e
  ...
  par-right-identity-≈e : f ∥ id⤨ {0} ≈e f
